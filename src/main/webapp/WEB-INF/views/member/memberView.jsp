...
$(function(){
	...
	$('#OutPlease').click(function(){
		...
		if(con == false){
		  return false;
			...
			alert(mem_id);
			location.href='${pageContext.request.contextPath}/member/pleaseOut.do?mem_id='+ mem_id;
		}
	});
	...
	$('#memUpdate').click(function(){
		...
		if(!$('input[name=mem_pass]').val().validationPWD()){
			alert('비밀번호은 영소문자, 숫자포함 4~8글자입니다.');
			$('input[name=mem_pass]').focus();
			return false;
		}
		...
		if(!$('input[name=mem_nickname]').val().validationCN()){
			alert('별명은 한글 최소 2글자에서 최대 10글자로 입력해주세요.');
			$('input[name=mem_nickname]').focus();
			return false;
		}
        ...
        <div class="headerSubandContent">
            ...
            <div class="box-signup">
                <form class="join" action="${pageContext.request.contextPath}/member/updateMemberInfo.do" method="post">
                    ...
                    <div class="submitBtn">
                        <input class="btn-simple" type="button" id="OutPlease" value="탈퇴요청">
